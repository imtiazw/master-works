...
bool FileSystem::CreateDirectory(char *name) {
    ...
    directory = new Directory(NumDirEntries);  
 	directory->FetchFrom(directoryFile);    
        directory->FetchFrom(directoryFile); // make directory in the current sector (0) 
	int originalSector= directory->getCurrentSector();;

    if(strlen(name) > SizePathNameMax) {
        printf("Name of directory too long\n");
        success = FALSE;
    }
    else if (directory->IsFull()) {
...
//	    Remove it from the dDirectoryFileSizeirectory
...
{ 
	...
	//	Open Directory to be able to remove files 
    ...
    if (fileHdr->IsDirectoryHeader()) {
    	if (!Dir->IsEmpty()) {
                ...
                delete DirFile;
                delete Dir;
                printf("The directory is the root, can't be delete\n");
                return FALSE;
     	}
...
void FileSystem::RoutePath(char *PathName, char** args, int *NumArgs)
{
    char *NameArgs[NumPathNamesMax];
    char **cp;
    char *wbuf;
    int i, j;
    ...
    for(cp=NameArgs; (*cp=strsep(&wbuf, "/")) != NULL ;){
        if ((*cp != '\0') && (++cp >= &NameArgs[NumPathNamesMax]))		//Separeting the names in the path  and just
            break;
    }


    for (j=i=0; NameArgs[i]!=NULL; i++){
        if(strlen(NameArgs[i])>0)
            args[j++]=NameArgs[i];
    }
 printf("%d\n", j);
    *NumArgs=j;
