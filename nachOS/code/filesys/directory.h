...
#ifdef CHANGED
 Directory(int size, int sector, int parentSector); 			// Initialize an directory
									// with space for "size", with sector and parent of the sector files
bool IsFull();		//no space for disk
bool IsRoot();		//is the parent

void MakeDirHierarchy(int sector, int parentSector); 	// 	create the new Directory with the sector and the parent

int getCurrentSector();	// get the actual position of the sector in table[]

bool IsEmpty(); 	//empty directory
