...
{
  if ((x1 == x2)&&(y1==y2)) return;
  fprintf(f, "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\" style=\"stroke:pink;stroke-width:1;fill-opacity:0.1;stroke-opacity:0.9\"/>\n",
      x1, y1, x2-x1, y2-y1, svg_color);
}

void svg_simple_rect(FILE *f, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2)
{
	if ((x1 == x2)&&(y1==y2)) return;
	  fprintf(f, "<rect x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"%s\" style=\"fill-opacity:0.5;stroke-opacity:0.9\"/>\n",
	      x1, y1, x2-x1, y2-y1, svg_color);
}
